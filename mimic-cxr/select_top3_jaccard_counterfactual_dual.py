...
if __name__ == "__main__":
    # 加载图像和文本特征
    with open(r"C:\Work\pricai\mimic-cxr\mimic_uid_to_image_features.pkl", "rb") as f:
        uid_to_img = pickle.load(f)
    with open(r"C:\Work\pricai\mimic-cxr\mimic_uid_to_text_features.pkl", "rb") as f:
        uid_to_txt = pickle.load(f)

    # 加载原始结构化 JSON
    with open(r"C:\Work\pricai\mimic-cxr\mimic_test\mimic_findings_structure_gpt54_dict.json", "r", encoding="utf-8") as f:
    ...
    with open("mimic_sim_mapping_top3_symbolic_442.json", "w", encoding="utf-8") as f:
        json.dump(sim_results, f, indent=4, ensure_ascii=False)

    # 2. 保存反事实编辑后的 3 个结构化样本 (Golden Exemplars)
    with open("mimic_dual_counterfactual_top3_442.json", "w", encoding="utf-8") as f:
