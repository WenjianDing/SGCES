...
example_mapping_path = "/root/autodl-tmp/ct_project/jmid_sim_mapping_top3_symbolic_64.json"
# 新增：反事实编辑后的结构化特征字典路径
example_edited_struct_path = "/root/autodl-tmp/ct_project/jmid_dual_counterfactual_top3_64.json"

save_data_path = "/root/autodl-tmp/ct_project/jmid_qwen3vl8b_ours_results.json"
...
**Task**
The provided radiological Findings are written in Japanese. Synthesize them into a concise, accurate radiological Impression written in Japanese.

**Guidelines**
- Language: Output the Impression in Japanese. Do not translate it into English.
- Terminology: Use standard Japanese radiological terminology. Common medical abbreviations such as MRI, CT, DWI, CRM, EMVI, MRF, T1, and T2 may be kept in English when clinically appropriate.
- Faithfulness: Base the Impression only on the provided Findings and the clinically aligned information in the examples. Do not add unsupported diagnoses.
- Style: Closely follow the style, length, and logical deduction pattern of the examples.
- Structural Alignment: Each historical example contains an "Aligned Core Entities (JSON)" block. The JSON keys are in English, but their values may be Japanese. Use this block as a reasoning anchor to identify the core clinical elements that genuinely correspond to the current case, while ignoring unaligned noise in the original reference text.
- Output: Provide ONLY the Japanese Impression text. Do not include explanations, labels, markdown, or chatty text.
